#!/bin/ksh
#-------------------------------------------------------------------------
#  File:  smr_load_wh_ship.ksh
#
#  Desc:  UNIX shell script to Process WH shipments into RMS from the Interface tables             
#-------------------------------------------------------------------------
#############################################################################
...
#2.	Call Function SMR_WH_SHIP_SQL.F_INIT_WH_SHIPMENTS to initiate the process and copy required data to RMS staging tables. 
#  As part of this function the shipment data from EDI table SMR_RMS_INT_SHIPPING will be loaded into SMR_WH_ASN for 
#   further validation and processing.
#3.	Invoke the package SMR_WH_SHIP_SQL.F_PROCESS_WH_SHIPMENTS to process the data from staging table.
#4.	Invoke the package SMR_WH_SHIP_SQL.F_FINISH_PROCESS to finish processing the data and update status accordingly. 
...
plsql_result=`sqlplus -s $dbConnect <<-ENDSQL 
   ...
   BEGIN
	  ...
	  if  SMR_WH_SHIP_SQL.F_INIT_WH_SHIPMENTS(l_ERROR_MESSAGE ) = FALSE then
		 raise_application_error(-20010, l_error_message);
	  else
		 L_error_message := '';
	  end if;

	  if  SMR_WH_SHIP_SQL.F_PROCESS_WH_SHIPMENTS(l_ERROR_MESSAGE ) = FALSE then
		 raise_application_error(-20010, l_error_message);
	  else
		 L_error_message := '';
	  end if;
	  
	  if  SMR_WH_SHIP_SQL.F_FINISH_PROCESS(l_ERROR_MESSAGE ) = FALSE then
		 raise_application_error(-20010, l_error_message);
	  else
		 L_error_message := '';
	  end if;
...
errorfile="err.smr_load_wh_ship.`date +"%h_%d"`"
