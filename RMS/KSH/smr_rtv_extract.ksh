#!/bin/ksh
#-------------------------------------------------------------------------
#  File:  smr_rtv_extract.ksh
#
#  Desc:  UNIX shell script to extract RTV data from SMR_RMS_RTV_STG  
...
# ==== =========== ================   =============
# 1.00 10-May-15   Murali Natarajan   Fetch the WH RTV's created in RMS from  SMR_RMS_RTV_STG      
...
#2.	 Call the Function SMR_LEAP_INTERFACE_SQL.RTV_EXTRACT to load the RTV interface table
#############################################################################
# Revision : $Id: 
...
plsql_result=`sqlplus -s $dbConnect <<-ENDSQL 
   ...
   BEGIN
	  ...
	  if  SMR_LEAP_INTERFACE_SQL.RTV_EXTRACT(l_ERROR_MESSAGE ) = FALSE then
...
errorfile="err.smr_rtv_extract.`date +"%h_%d"`"
