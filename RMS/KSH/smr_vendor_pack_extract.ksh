#!/bin/ksh
#-------------------------------------------------------------------------
#  File:  smr_vendor_pack_extract.ksh
#
#  Desc:  UNIX shell script to extract RTW data from SMR_RMS_PACK_DTL_STG  
...
# ==== =========== ================   =============
# 1.00 10-May-15   Murali Natarajan   Fetch the Vendor pack Created in RMS from SMR_RMS_PACK_DTL_STG      
#                                     and populate the interface tables
#This script will be the main script to extract the data from the staging table and populate the RTV interface table. 
#1.	Invoked from UC4 Job.
#2.	 Call the Function SMR_LEAP_INTERFACE_SQL.PACK_DTL_EXTRACT to load the Pack interface table
...
plsql_result=`sqlplus -s $dbConnect <<-ENDSQL 
   ...
   BEGIN
	  ...
	  if  SMR_LEAP_INTERFACE_SQL.PACK_DTL_EXTRACT(l_ERROR_MESSAGE ) = FALSE then
...
errorfile="err.smr_wh_receiving.`date +"%h_%d"`"
